...
def source(tmp_path):
    ...
    for minute in (15, 20):
        ...
        table = pa.table({
            "symbol": pa.array([f"S{i // 512:04d}" for i in range(rows)], type=pa.large_string()),
            "event_time": pa.array(times, type=pa.timestamp("us")),
            "time_int": [91_500_000 + (minute - 15) * 100000 + i % 512 for i in range(rows)],
            "value": [i for i in range(rows)],
            "unused": [hashlib.sha256(str(i).encode()).hexdigest() for i in range(rows)],
        })
        path = root / f"bucket-{minute}.parquet"
        pq.write_table(table, path, row_group_size=512, compression="zstd")
        meta = pq.read_metadata(path)
        entries.append(ObjectEntry(object_id=f"bucket-{minute}", dataset="snapshots", trade_date="2026-09-04",
            bucket_start=f"2026-09-04T09:{minute}:00.000+08:00", bucket_end=f"2026-09-04T09:{minute + 5}:00.000+08:00",
            relative_path=path.name, bytes=path.stat().st_size, rows=rows,
            uncompressed_bytes=sum(meta.row_group(i).total_byte_size for i in range(meta.num_row_groups)),
            source_fingerprint="test", version="r1"))
...
def running(source, tmp_path, *, handler=MarketDataGatewayHandler, tokens=None):
    root, _ = source
    server, thread = _start_gateway(root, handler=handler, user_tokens=tokens, auth_required=bool(tokens))
    service = DataService(gateway_host="127.0.0.1", gateway_port=server.server_port,
        gateway_token=next(iter(tokens.values())) if tokens else None, cache_root=tmp_path / "cache",
        limits=ServiceLimits(user_cores=2, read_options=ReadOptions(coalesce_gap_bytes=0, seek_cost_bytes=0)))
    try:
...
def query(**kwargs):
    raw = dict(dataset="snapshots", start="2026-09-04T09:15:00.100+08:00",
        end="2026-09-04T09:25:00+08:00", symbols=["S0002", "S0020"], columns=["value", "event_time"])
    raw.update(kwargs)
...
def test_http_validation_before_success_headers(source, tmp_path):
    ...
    with running(source, tmp_path) as (service, _):
        ...
        payload = dict(dataset="snapshots", start="2026-09-04T09:15:00+08:00", end="2026-09-04T09:20:00+08:00")
        assert client.post("/v1/data", json=dict(payload, stocks=["S0002"])).status_code == 422
        assert client.post("/v1/data", json=dict(payload, columns=["bad_column"])).status_code == 422
        assert client.post("/v1/data", json=dict(payload, symbols=[])).status_code == 422
        response = client.post("/v1/data", json=dict(payload, symbols=["NO_MATCH"], columns=["symbol"]))
...
def test_standard_http_ranges_auth_bounds_and_version(source, tmp_path):
    with running(source, tmp_path, tokens={"a": "test-token"}) as (_, server):
        entry = source[1][0]
        url = "/v1/object?" + urllib.parse.urlencode({k: getattr(entry, k)
        ...
        response.read()
        for header, status, expected in [({"Range": "bytes=0-3"}, 206, b"PAR1"),
...
def test_four_users_native_clients(source, tmp_path):
    ...
    with running(source, tmp_path, tokens=tokens) as (_, server):
        def worker(i):
            with MarketDataClient.connect(gateway_host="127.0.0.1", gateway_port=server.server_port,
                gateway_token=f"token{i}", cache_root=tmp_path / f"native-{i}", cores=1,
                read_options={"coalesce_gap_bytes": 0, "seek_cost_bytes": 0}) as client:
                table = client.read_table({"dataset": "snapshots", "start_date": "2026-09-04",
                    "end_date": "2026-09-04", "daily_start": "09:15", "daily_end": "09:25",
                    "symbols": [f"S{i:04d}"], "columns": ["symbol", "value"]})
...
def test_coalescing_and_query_validation():
    ...
    with pytest.raises(ValueError):
        query(symbols="S0001")
    with pytest.raises(ValueError):
        DataRequest.from_query(dict(dataset="snapshots", start="2026-09-04", end="2026-09-05", stocks=["S0001"]))
