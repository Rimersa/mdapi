...
{
  printf 'MDAPI_DATA_ROOT=%s\n' "${SERVER_DATA_ROOT}"
  printf 'MDAPI_GATEWAY_HOST=10.10.10.87\n'
  printf 'MDAPI_GATEWAY_PORT=18787\n'
  printf 'MDAPI_MAX_STREAMS=2\n'
  printf 'MDAPI_QUEUE_TIMEOUT=300\n'
  printf 'MDAPI_TOKEN_FILE=%s\n' "${SERVER_TOKEN_FILE}"
} >"${SERVER_CONFIG_TEMP}"
if [[ "${SERVER_INSTALL_MODE}" == "system" ]]; then
...
for _ in {1..40}; do
  if /usr/bin/python3 -c \
    'import json,urllib.request; print(json.load(urllib.request.urlopen("http://10.10.10.87:18787/health", timeout=1)))' \
    >/dev/null 2>&1; then
    SERVER_HEALTH_OK=1
    break
  fi
...
echo
echo "服务器安装完成: http://10.10.10.87:18787"
